...
# class TestSubCategoryParameters(generics.ListAPIView):
#     permission_classes = (IsAuthenticated,)
#     authentication_classes = (BasicAuthentication, TokenAuthentication)

#     def get(self, reqeust):
#         try:
#             test_sub_category_id = reqeust.GET.get("test_sub_category_id", None)
#             if not test_sub_category_id:
#                 raise Exception("Please pass test_sub_category_id ")
#             paramters = Paramters.objects.filter(test_sub_category_id=test_sub_category_id)
#             if not paramters:
#                 raise Exception(f"no Prameters found with test_sub_category_id {test_sub_category_id}")
            
#             result = {}
#             for param in paramters:
#                 df = self.get_file_dataframe(param)
...
#             df = df[final_mask]
#             req_params = parameter.req_params or ['multicast_group_increment', 'unit', 'vlan_tag', 'protocol', 'group_name']
#             df = df[req_params]
...
#     def get_file_dataframe(self,parameter):
#         try:
#             bucket = "genaidev" #parameter.bucket 
#             s3_key = "r0_protocols_protocols.csv" # parameter.s3_key 
...
#             'exact': lambda df, key, value: df[key] == value,
#             'iexact': lambda df, key, value: self.safe_str_accessor(df[key], 'lower') == value.lower(),
#             'notin': lambda df, key, value: ~df[key].isin(value),
#             'notequal': lambda df, key, value: df[key] != value,
#             'startswith': lambda df, key, value: self.safe_str_accessor(df[key], 'startswith', value),
#             'istartswith': lambda df, key, value: self.safe_str_accessor(df[key], 'lower').str.startswith(value.lower()),
#             'endswith': lambda df, key, value: self.safe_str_accessor(df[key], 'endswith', value),
#             'iendswith': lambda df, key, value: self.safe_str_accessor(df[key], 'lower').str.endswith(value.lower()),
#             'contains': lambda df, key, value: self.safe_str_accessor(df[key], 'contains', value, regex=True),
#             'icontains': lambda df, key, value: self.safe_str_accessor(df[key], 'lower').str.contains(value.lower(), regex=True),
#             'regex': lambda df, key, value: self.safe_str_accessor(df[key], 'match', value)
#         }

#             check_type = condition['check_type']
#             if check_type in condition_functions:
#                 return condition_functions[check_type](df, condition['check_key'], condition['check_value'])
#             else:
...
#             and_conditions = [self.apply_conditions(df, cond) if 'conditions' in cond else self.check_condition(df, cond) for cond in conditions['and']]
#             return pd.concat(and_conditions, axis=1).all(axis=1)
#         elif 'or' in conditions:
#             or_conditions = [self.apply_conditions(df, cond) if 'conditions' in cond else self.check_condition(df, cond) for cond in conditions['or']]
#             return pd.concat(or_conditions, axis=1).any(axis=1)
#         else:
#             return self.check_condition(df, conditions)
